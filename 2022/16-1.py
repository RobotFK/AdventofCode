...
for path in paths : # Adding the Pressurervalue at index 0, moving all others 1 futher
    ...
    for vertex in path:
        ...
        time -= (traveltime+1) #Subtract the time it took to reach this vertex and to turn on the valve
        flow = next(valve[1] for valve in valves if valve[0] == vertex) # How much flows through the current vertex(current valve)
        pressure_release += int(flow)*time #Add the total flow this valve will release to the sum pressure_release
        prev_valve = vertex
    pressure.append(pressure_release)
