...
void D_9_2(){
	...
	for(auto&seq:sequences){//For each Sequence
		int prev=0;
		for(int i= seq.differentials.size()-1;i>=0;i--){
			//cout<< i<<": ";
			prev=(seq.differentials[i].front()-prev);
			//cout << prev<<endl;
		}
		seq.numbers.front()-prev;
		//cout<< "Prev: "<<seq.numbers.front()-prev<<endl<<endl;
		sum+=seq.numbers.front()-prev;
	}
