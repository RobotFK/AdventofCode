...
$rt = Get-PermutationsTime $permuatations[0]#Slowest Route time of a pair
1..(($permuatations.Length)-1)|%{
$sum =  Get-PermutationsTime $permuatations[$_]
if($sum -ge $rt){$rt = $sum;Write-Host "Permutaion $($_) is new slowest ($($rt))"}
}
Write-Host "Slowest Roundtrip takes $($rt) Time"