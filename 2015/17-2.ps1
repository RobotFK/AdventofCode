...
$containercount = @(0)*($con.Length)
foreach($combination in $combinations){
    $sum = 0
    0..($combination.length)|%{if($combination[$_] -eq '1'){$sum++}}
    $containercount[$sum]++
}

$mincontainercount = 0
($containercount.Length-1)..0|%{if($containercount[$_] -ne 0){
$mincontainercount = $containercount[$_]}}
Write-Host $mincontainercount
